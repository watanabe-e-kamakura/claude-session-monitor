...
# 注意: 変数展開の直後に全角文字を置かないこと（macOS bash 3.2 は ${VAR} ブレース必須）
set -euo pipefail
...
if [ -z "$NODE_BIN" ]; then
  echo "✗ node が見つかりません（Node.js 22.5+ が必要）"; fail=1
else
  ver="$("$NODE_BIN" -v | sed 's/^v//')"
  major="${ver%%.*}"
  if [ "$major" -lt 22 ]; then
    echo "✗ Node.js 22.5+ が必要です（現在 ${ver}）"; fail=1
  else
...
if [ -z "$CLAUDE_BIN" ]; then
  echo "✗ claude CLI が見つかりません（必須: セッション一覧取得に使用）"; fail=1
else
  echo "✓ claude (${CLAUDE_BIN})"
  echo "  ※ claude にログイン済みであること（未ログインだとセッション一覧が空になります）"
fi

WEZTERM_BIN="$(find_bin wezterm)"
if [ -z "$WEZTERM_BIN" ]; then
  echo "△ wezterm なし（任意: 「飛ぶ」「新規起動」「送信」が無効になる。brew install --cask wezterm）"
else
...
GH_BIN="$(find_bin gh)"
if [ -z "$GH_BIN" ]; then
  echo "△ gh なし（任意: PR レビューボードが無効になる。brew install gh && gh auth login）"
elif ! "$GH_BIN" auth status >/dev/null 2>&1; then
  echo "△ gh が未ログイン（PR レビューボードが空になります。gh auth login を実行してください）"
else
  echo "✓ gh (${GH_BIN}) ログイン済み"
fi

if [ "$fail" = 1 ]; then
  echo "必須依存が不足しています。導入後に再実行してください。"
  exit 1
fi
...
if [ ! -f "$CONF" ]; then
  ...
  cat > "$CONF" <<'JSON'
{
  "_comment": "Claude Session Monitor 設定。キーは src/config.js 参照。環境変数 ORCH_* が優先。空文字は未設定扱い。",
  "defaultCwd": "",
  "workspaceRoot": "",
...
if curl -sf -o /dev/null "http://127.0.0.1:4317/api/live"; then
  ...
  echo "  （ポートを 4317 から変更している場合はこの確認が誤検知になります）"
  exit 1
fi
